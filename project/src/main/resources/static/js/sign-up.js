$(document).ready(function () {
	...
	$('#sign-up__form__department-field').change(function () {
		if ($(this).children('option:selected').val() === '') {
			$('#sign-up__form__team-leader-field').html(
			...
			$.getJSON(
				...
				function (data) {
					...
					for (let i = 0; i < len; i++) {
						html += '<option value="' + data[i].value0 + '">' + data[i].value1 + '</option>';
					}
					html += '</option>';
